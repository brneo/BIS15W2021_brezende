...
```{r}
ggplot(data=iris, mapping=aes(x=Species, y=Petal.Length))+geom_boxplot() #look at cheatsheet for geoms
#range of values so make a boxplot
```
...
```{r}
ggplot(data=iris, mapping=aes(x= Sepal.Width, y=Sepal.Length, ))+ #could also reverse the specification 
  geom_point()
```
...
```{r}
naniar::miss_var_summary(homerange) #tells us how NA represented and where they're concentrated
```
...
ggplot(data = homerange, mapping = aes(x = log10.mass, y = log10.hra)) +
  geom_jitter() #back pocket tool, if plot needs "noise" to see things better
```
...
#se will add the 95 confidence level 
```
...
```{r}
ggplot(data=homerange, mapping=aes(x=log10.hra, y=log10.preymass))+geom_point(na.rm=T) #adding = F tells us how ggplot is dealing with NAs, but T will remove the warning 
```
...
```{r}
homerange %>% 
  ggplot(aes(x=trophic.guild)) + #so common to add pipe, make sure you use "+"
  geom_bar()
```
...
homerange %>% 
  ...
  geom_col() #geombar would not work because of the aesthetic. so use col instead 
```
...
homerange %>% 
  ...
  geom_col() #can produce mini tbl and get a bar graph. 
```
...
```{r}
glimpse(homerange) #if we're still confused we can use glimpse to look at everything 
```
...
homerange %>% 
  filter(class=="mammalia") %>% 
  top_n(-10, log10.mass) %>% #top_n grabs bottom 10 values (neg or pos)
  ggplot(aes(x=common.name, y=log10.mass))+
  geom_col()
```
...
homerange %>% 
  filter(class=="mammalia") %>% 
  top_n(-10, log10.mass) %>% 
  ggplot(aes(x=reorder(common.name, log10.mass), y=log10.mass))+ #reorder
  geom_col()+
  coord_flip()
```
