...
```
```{r}
head(sug$pheno) #1 in the sex means they're only using the males 
```
...
plotMissing(sug, main="", reorder=1) #homework example, you'll see that researcher only does reordering on some genotypes 
#summary(sug)
...
```{r}
summary(out.em, threshold=3) #showing who has the LOD score above 3
```
We can plot the results as follows.
```{r}
plot(out.em)
plot(out.em, chr=c(7,15), ylab="LOD Score") #specifies that we only want to see chromosome 7 and 15
```
### iplotScanone
