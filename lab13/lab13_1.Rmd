...
```{r, eval=F, echo=T}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), #select variable is the drop down menu
...
ui <- fluidPage(
    ...
    selectInput("y", "Select Y Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), 
              selected = "bill_length_mm") #can select with both X and Y axis
)
...
shinyApp(ui, server) #long window helps to see entire line of code
```
...
server <- function(input, output) {
  output$plot <- renderPlot({
    ggplot(penguins, aes_string(x = input$x, y = input$y, color="species")) + geom_point() + theme_light(base_size = 18) #render plot is the reactive expression to make a plot in the app, the input needs the $ to label the x and y axis
  })
