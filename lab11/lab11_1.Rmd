...
deserts %>% 
  ...
  summarise(n=n()) #tells you how many observations are made for each species. Data is tidy. Can easily plot this group. 
```
...
homerange %>% 
  ...
  labs(title = "Distribution of Body Mass") #alpha is for transparency so readers can see the lines behind the graph
```
...
homerange %>% 
  ...
  summarise(min=min(log10.mass),
            max=max(log10.mass)) #gives you an idea of the range of distribution, now think about how to break them up 
```
```{r}
