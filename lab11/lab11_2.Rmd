...
p+theme_linedraw()+
  ...
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
       y= "n",
       fill= "Trophic Guild") #this line is the title of the legend
```
...
```{r}
p2+ facet_wrap(~taxon) #make a wrap by "~" taxon, which is a categorical variable 
#end result is density plot for all the taxa at once. similar to a grouping function. 
```
```{r}
p2+ facet_wrap(~class)
```
...
```{r}
p2+facet_grid(thermoregulation~class) #no mammals or birds are ectotherms 
#demonstrates shared 
```
