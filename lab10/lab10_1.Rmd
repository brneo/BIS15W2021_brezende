...
```{r message=FALSE, warning=FALSE}
life_history <- read_csv(here("lab10", "data", "mammal_lifehistories_v2.csv")) %>% na_if("-999") %>% clean_names #common string used when importing data
```
...
```{r}
life_history %>% 
  count(order, sort=T) #sort T will function similar to arrange
```

```{r}
life_history %>% 
  ggplot(aes(x=order)) + geom_bar()+coord_flip() #only specified X, asking R to look at variables together
```

```{r}
life_history %>% 
  group_by(order) %>% #putting all observations together
  summarize(mean_mass=mean(mass, na.rm=T)) #removing NA's with the = T, generates mean mass 
```

```{r}
options(scipen=999) #cancels scientific notation for the session only, not permanently 
```
...
```{r}
life_history %>% #code chunk from above, adding some adjustments 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=order, y=mean_mass))+geom_col()+coord_flip()+scale_y_log10() #scaling the results in order to get a cleaner plot, makes a lot more sense
```

## Scatterplots
Scatter plots allow for comparisons of two continuous variables.
```{r}
life_history %>% #allows us to compare two continuous variables (ex: wean_mass and gestation)
  ggplot(aes(x=gestation, y=wean_mass)) + geom_point(na.rm=T) + scale_y_log10() + geom_smooth(method="lm") #LM means linear model
```
...
```{r}
life_history %>% 
  ggplot(aes(x=order, y=mass))+geom_boxplot()+scale_y_log10()+coord_flip() #we know mass will be problem so scale 
```
...
life_history %>% 
  ...
  ggplot(aes(x=family, y=mass))+geom_boxplot(na.rm=T)+scale_y_log10()+coord_flip() #have the na.rm in order to remove the error 
```
...
elephants %>% 
  ...
  labs(title = "Elephant Age vs. Height",
       x = "Age",
       y = "Height") +
  theme(plot.title = element_text(size = 12, face = "bold"), #customizable and adjustments are endless 
        axis.text = element_text(size = 10),
        axis.title = element_text(size = 10)) #adds theme which allows us to change font, size, and color
```
...
```{r}
elephants %>% 
  ggplot(aes(x=age, y=height)) + geom_point(color="steelblue") + geom_smooth(method=lm, se=F) +
  labs(title = "Elephant Age vs. Height",
       x = "Age",
       y = "Height") +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))#h just is for left, center, right
#aesthetics can be added to any or all plots. promotes consistency and clean data. 
```
...
```{r}
elephants %>% 
  ggplot(aes(x=sex, fill=sex))+geom_bar()+ #fill = 
  labs(title = "# Elephants by Sex",
       x = "Sex",
       y = NULL) + #will remove the entire Y axis label. keeps numbers or variables. 
...
life_history %>% 
  ...
  geom_point() #bigger individuals get bigger sized dots which demonstrates scaling. plots can be very exploratory 
```
...
homerange %>% 
  ...
  geom_bar()+
  labs(title = "# taxa by Thermoregulatin Type",
       x = "Thermoregulation",
       y = "n") + #will remove the entire Y axis label. keeps numbers or variables. 
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.mass, y=log10.hra, size=mean.mass.g))+geom_point()+ #as mass increases the homerange increases. made by using size function. if you want to change size for all the points then put size operator into the geom point. 
  labs(title = "Mass vs Homerange",
       x = "log10.mass",
       y = "log10.hra") + #will remove the entire Y axis label. keeps numbers or variables. 
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
