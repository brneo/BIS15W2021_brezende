...
```{r}
p <- homerange %>% 
  ggplot(aes(x= log10.mass, y= log10.hra)) #storing a blank plot as p so we can manipulate aesthetics and different plots 
```

Play with point size.
```{r}
p +geom_point(size=1) #size manipulates the dots, can adjust accordingly 
```

Map shapes to another categorical variable.
```{r}
p+geom_point(aes(shape=thermoregulation, color=thermoregulation), size=1.75) #shapes can be manipulated and change, can control style of shape
```
...
```{r}
homerange %>% 
  ggplot(aes(x = taxon)) + geom_bar() + #bc it's a straight count
  coord_flip() +
...
```{r}
homerange %>% 
  ggplot(aes(x = taxon, fill = trophic.guild)) + geom_bar() + #use the fill command with another categorical variable. 
  coord_flip() +
  labs(title = "Observations by Taxon in Homerange Data",
       x = "Taxonomic Group",
       fill = "Trophic Guild")
...
homerange %>% 
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + #slanting the text, cleaning up the x. makes huge improvement for readers 
  labs(title = "Observations by Taxon in Homerange Data",
       x = "Taxonomic Group",
       fill= "Trophic Guild")
```
...
```{r}
penguins %>% 
  ggplot(aes(x=island, fill=species)) + geom_bar(position="dodge") #shows counts of penguins per island 
```
...
```{r}
homerange %>% 
  ggplot(aes(x = class, y = log10.mass, group=taxon, fill=taxon)) + #end up with different groups. however, group will NOT color anything in. 
  geom_boxplot()
```

```{r}
homerange %>% 
  ggplot(aes(x = class, y = log10.mass, group=taxon, color=taxon)) + #shows outline instead of filling 
  geom_boxplot()
```
