...
penguins %>% 
  group_by(island) %>% 
  summarize(mean_body_mass_g=mean(body_mass_g), #making a new name
            n=n()) #count keeps it real
```
...
```{r}
penguins %>% 
  group_by(island, species) %>% #reversing the order gets a different reordering of columns
...
```{r}
penguins %>% 
  count(island, sort = F) #sort=T sorts the column in descending order
...
```{r}
penguins %>% 
  tabyl(island, species) #tabyl adorn operator
```
