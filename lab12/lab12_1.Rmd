...
library(here)
library(janitor)
library(lubridate) #specifically calling this one
```

Now we can list all of the files in a directory. This isvery helpful when you are working with multiple files at once.
```{r}
files <- list.files(path = "data/spiders", pattern = ".csv") #specify path, the path is different depending on you computer. 
files
```
...
```{r, message = FALSE}
spider_list <- lapply(files, read_csv) #list can store many different types of objects at the same time; using files object we made earlier
```
...
```{r}
names(spider_list[[3]]) #conveninent when it comes to merging these files together 
```
...
names_list <- strsplit(names, split = " .csv") #want to keep the first element of rach list
names_list
```
...
names(spider_list) #spider names are alot more clearherew
```

## Practice
1. Now that our list elements are named, how could we access the Butte County data by name?
```{r}
spider_list$Butte #pulls up all the butte county data and clean names fr objects in our list. 
```
...
```{r}
spiders_all <- bind_rows(spider_list) #will match all columns by name and stick them together
spiders_all
```
...
full_exampleDF #retaining everything 
```
...
anti_exampleDF #telling you it's not present in table B!! 
```
...
```{r}
spiders_with_locs <- left_join(spiders_all, spiders_locs, by = c("Accession")) #left-join because 
spiders_with_locs
```
