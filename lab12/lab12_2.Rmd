...
spiders %>% 
  ...
  summary() #the min and max help us build our bounding box, then concatenate the min and max. use the values from the table
```
...
lat <- c(34.67, 41.80)
long <- c(-124.1, -115.5)
bbox <- make_bbox(long, lat, f = 0.05) #f is a way of slightly expanding the box past coordinates
```
...
```{r}
library(albersusa) #cleaner when working with lower 48 states
```
...
2. Now adjust the plot about to give a different color for specimens that occur in caves.
```{r}
ggplot() +
  geom_sf(data = ca_comp, size = 0.125)+
  geom_sf(data = ca_cnty_comp, size = 0.125)+
  geom_point(data = spiders, aes(longitude, latitude, color=troglo), shape=3)+
  labs(x = "Figure 1: Distribution of Usofila spiders")+
  theme_minimal() +
  theme(axis.text = element_blank()) +
  theme(axis.title.x = element_text(hjust=0.5, size = 10)) +
